...
from vis.output import prep_result_vis, animate_scene, make_image_grid_2x2
...
def run_vis(
    ...
    **kwargs,
    ...
    # save input frames
    inp_img_path = save_input_frames(
        dataset,
        f"{save_dir}/{dataset.seq_name}_input.png",
        fps=cfg.fps,
        overwrite=True,
    )
    ...
    out_ext = "/" if render_layers or save_frames else ".png"
    phase_results = {}
    phase_max_iters = {}
    for phase in phases:
    ...
    if make_grid:
        for phase, it in phase_max_iters.items():
            grid_path = f"{save_dir}/{dataset.seq_name}_{phase}_grid.png"
            img_paths = [
                inp_img_path,
                f"{save_dir}/{dataset.seq_name}_{phase}_final_{it}_src_cam.png",
                f"{save_dir}/{dataset.seq_name}_{phase}_final_{it}_above.png",
                f"{save_dir}/{dataset.seq_name}_{phase}_final_{it}_side.png",
            ]
            make_image_grid_2x2(
                grid_path,
                img_paths,
