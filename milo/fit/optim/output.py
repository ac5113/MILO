...
def save_input_frames(dataset, out_path, fps=30, overwrite=False):
    if not overwrite and os.path.isfile(out_path):
        return out_path

    paths = dataset.sel_img_paths
    if len(paths) == 1:
        # single-image: write the input frame as a PNG still
        imageio.imwrite(out_path, imageio.imread(paths[0]))
        print(f"SAVED INPUT IMAGE TO {out_path}")
        return out_path
    ...
    writer.close()
    print(f"SAVED {len(paths)} INPUT FRAMES TO {out_path}")
    return out_path
