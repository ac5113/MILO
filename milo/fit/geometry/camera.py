...
def perspective_projection_depth(
    points, rotation, translation, focal_length, camera_center
):
    """
    This function computes the depth from the perspective projection of a set of points.
    Input:
        points (B, T, N, 3): 3D points
        focal_length (T, 2): Focal length
        camera_center (T, 2): Camera center
        rotation (B, T, 3, 3): OPTIONAL Camera rotation
        translation (B, T, 3): OPTIONAL Camera translation
    """
    # Transform points
    points = points.clone()
    points = torch.einsum("btij,btkj->btki", rotation, points)
